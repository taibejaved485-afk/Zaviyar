...
export default function Testimonials() {
  ...
  return (
    <section className="py-12 sm:py-16 bg-[#FCFAF8] transition-colors duration-300 relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-indigo-100/50 rounded-full blur-[140px] -mr-48 -mt-48 opacity-60 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-purple-100/50 rounded-full blur-[140px] -ml-48 -mb-48 opacity-60 pointer-events-none" />
      ...
      <div className="max-w-full mx-auto px-4 sm:px-8 lg:px-12 relative z-10">
        ...
        <div className="text-center w-full max-w-4xl mx-auto mb-10">
          <motion.div
            ...
            viewport={{ once: true }}
            className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-indigo-50 text-indigo-600 rounded-full text-xs font-bold tracking-[0.15em] uppercase mb-4 border border-indigo-100/80 shadow-xs"
          >
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8 bg-white p-3 rounded-2xl border border-slate-200/80 shadow-xs">
          ...
          <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200/60 ml-auto">
            <button
        ...
        {viewMode === 'spotlight' ? (
          ...
          <div className="relative w-full">
            <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/80 shadow-xl shadow-slate-200/60 p-6 sm:p-10 md:p-12 relative overflow-hidden w-full">
              ...
              <Quote className="absolute top-4 right-4 sm:top-8 sm:right-8 w-28 h-28 sm:w-44 sm:h-44 text-indigo-50 opacity-40 pointer-events-none" />
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 mb-6 pb-6 border-b border-slate-100">
                    ...
                    {activeStory.metric && (
                      <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-gradient-to-r from-emerald-50 via-teal-50 to-emerald-50 border border-emerald-200/80 rounded-xl shadow-2xs">
                  ...
                  {/* Main Quote Content */}
                  <blockquote className="text-lg sm:text-xl md:text-2xl text-slate-900 font-semibold leading-relaxed mb-8 italic tracking-tight">
                    "{activeStory.quote}"
                  </blockquote>

                  {/* Case Highlights Row */}
                  {activeStory.highlights && (
                    <div className="flex flex-wrap items-center gap-2 mb-8 p-3 bg-slate-50/80 rounded-xl border border-slate-200/60">
                      <span className="text-xs font-bold text-slate-500 uppercase tracking-wider mr-2">Impact Highlights:</span>
                      {activeStory.highlights.map((item, idx) => (
                        <span key={idx} className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 bg-white text-slate-700 rounded-lg border border-slate-200 shadow-2xs">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 pt-2">
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        <img 
                          ...
                          className="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl object-cover ring-2 ring-indigo-600/30 shadow-md"
                        />
                        <div className="absolute -bottom-1 -right-1 bg-indigo-600 text-white p-1 rounded-full border-2 border-white shadow-2xs">
                          <ShieldCheck size={12} />
                        </div>
                      </div>
                    ...
                    <div className="inline-flex items-center gap-2 text-xs font-bold text-indigo-700 bg-indigo-50 border border-indigo-100 px-3.5 py-2 rounded-xl">
                      <Building2 size={15} />
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 flex items-center justify-between">
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-indigo-600 transition-colors px-3 py-1.5 rounded-lg hover:bg-slate-100"
                >
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={prev}
                    className="p-2.5 bg-slate-100 hover:bg-indigo-600 hover:text-white rounded-xl text-slate-600 transition-all active:scale-95 border border-slate-200/60 shadow-2xs"
          ...
          <div className="grid md:grid-cols-2 gap-6 w-full">
            {filteredTestimonials.map((item, idx) => (
              <motion.div
                ...
                className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between group relative overflow-hidden hover:-translate-y-1"
              >
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between gap-3 mb-4">
                    <span className="text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 bg-indigo-50 text-indigo-700 rounded-lg">
                      {item.industry}
                    </span>
                    {item.metric && (
                      <span className="text-xs font-black text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-200/60 flex items-center gap-1">
                        <TrendingUp size={12} />
                  ...
                  <blockquote className="text-sm text-slate-800 font-medium leading-relaxed mb-6 italic">
                    "{item.quote}"
                  </blockquote>
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
                  ...
                  />
                  <div>
                    <h5 className="text-sm font-bold text-slate-900 leading-tight">{item.author}</h5>
                    <p className="text-xs text-indigo-600 font-medium">{item.role} · {item.company}</p>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        <div className="mt-12 w-full grid grid-cols-2 sm:grid-cols-4 gap-4">
          {AGENCY_STATS.map((stat, idx) => (
            <motion.div
              ...
              className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs text-center flex flex-col items-center justify-center group hover:border-indigo-300 hover:shadow-md transition-all duration-300"
            >
              <div className="p-2.5 bg-indigo-50 text-indigo-600 rounded-xl mb-3 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300 shadow-2xs">
                <stat.icon size={20} />
              </div>
              ...
              <div className="text-[11px] text-slate-400 font-normal mt-0.5">{stat.sub}</div>
            </motion.div>
          ))}
        </div>
      </div>
